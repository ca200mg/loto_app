...
class N4Edit extends StatefulWidget {
  final int id;
  const N4Edit({required this.id, Key? key}) : super(key: key);

  @override
  State<N4Edit> createState() => _N4EditState();
}

class _N4EditState extends State<N4Edit> {
  ...
  Future<void> _loadN4Nums() async {
    List<String> nums = await getN4Nums(widget.id);
    print('nums:$nums');
    setState(() {
      n4Nums = nums;
      print('n4Nums:$n4Nums');
    ...
    body: Column(
      children: [
        Padding(
                  ...
                  child: Text('第${n4Nums[0]}回   ${n4Nums[1]}'),
                ),
        Row(
          children: [
            // 1
                ...
                children: [
                  ElevatedButton(
                    onPressed: (numberList.length == 4) ? () async {
                      ...
                      await updateN4Data(widget.id, int.parse(n4Nums[0]), n4Nums[1], preNumSetList2, _selectedIndex);
                    ...
                    onPressed: (numberList.length == 4) ? () async {
                      // 選択した数字をMinilotoCheckに渡すなどの処理を行う
                      await deleteN4Record(widget.id);
                      Navigator.pop(context, true);
                      print('pressed');
                    } : null, // numberListの要素数が6でない場合、ボタンを無効にする
                    child: const Text('削除'),
...
Future<List<String>> getN4Nums(int id) async {
  // データベースのパスを取得
  ...
  // 指定された id のレコードを取得
  List<Map<String, dynamic>> result = await database.query('n4',
      columns: ['no', 'date', 'main1', 'main2', 'main3', 'main4', 'type'],
      where: 'id = ?',
      whereArgs: [id]);
...
Future<void> updateN4Data(int id, int no, String date, List<String> preNumSetList2, int selectedIndex) async {
    String path = join(await getDatabasesPath(), 'user_database.db');
    Database database = await openDatabase(path);
...
Future<void> deleteN4Record(int id) async {
  // データベースのパスを取得
  String path = join(await getDatabasesPath(), 'user_database.db');
  ...
  try {
    // 指定された id のレコードを削除
    await database.delete('n4', where: 'id = ?', whereArgs: [id]);
  } catch (e) {
