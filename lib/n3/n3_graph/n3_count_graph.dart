...
import 'package:loto_app/n3/n3_graph/n3_bar_graph.dart';
import 'package:sqflite/sqflite.dart';

class N3CountGraph extends StatefulWidget {
  const N3CountGraph({super.key});

  @override
  State<N3CountGraph> createState() => _N3CountGraphState();
}

class _N3CountGraphState extends State<N3CountGraph> {
        ...
        Column(
          children: [
            ...
            SizedBox(
              height: 200,
              width: 400,
              child: N3BarGraph(countKey: countKey, countValue: countValue,)),
            Switch(
              value: sortOrNot,
...
// n4テーブル内のmain1からmain6の各数字の出現回数をカウントする関数
Future<Map<String, int>> countMainNumbers() async {
  ...
  for (int i = 1; i <= 3; i++) {
    ...
    List<Map<String, dynamic>> result = await database.rawQuery(
      'SELECT main$i, COUNT(*) as count FROM n3 GROUP BY main$i',
    );

    // クエリの結果を解析し、数字ごとの出現回数をマップに追加
    for (final record in result) {
      final number = record['main$i'].toString(); // 数字 (文字列)
