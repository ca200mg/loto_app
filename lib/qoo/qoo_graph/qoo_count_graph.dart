...
import 'package:loto_app/qoo/qoo_graph/qoo_bar_graph.dart';
import 'package:sqflite/sqflite.dart';

class QooCountGraph extends StatefulWidget {
  const QooCountGraph({super.key});

  @override
  State<QooCountGraph> createState() => _QooCountGraphState();
}

class _QooCountGraphState extends State<QooCountGraph> {
        ...
        Column(
          children: [
            ...
            SizedBox(
              height: 200,
              width: 400,
              child: QooBarGraph(countKey: countKey, countValue: countValue,)),
            Switch(
              value: sortOrNot,
...
// qooテーブル内のmain1からmain6の各数字の出現回数をカウントする関数
Future<Map<String, int>> countMainNumbers() async {
  ...
  for (int i = 1; i <= 4; i++) {
    ...
    List<Map<String, dynamic>> result = await database.rawQuery(
      'SELECT main$i, COUNT(*) as count FROM qoo GROUP BY main$i',
    );
