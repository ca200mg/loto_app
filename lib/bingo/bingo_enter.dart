...
class _BingoEnterState extends State<BingoEnter> {
  List<int?> numberList = List.generate(8, (index) => null); // null初期化
  double height = 60;
  double width = 300;

  Widget buildGridView(int startIndex, int listIndex) {
    return SizedBox(
      height: height,
      width: width,
      child: GridView.builder(
        ...
        itemBuilder: (BuildContext context, int index) {
          int number = startIndex + index;
          return GestureDetector(
            onTap: () {
              setState(() {
                numberList[listIndex] = number;
              });
            },
            child: Container(
              decoration: BoxDecoration(
                color: numberList[listIndex] == number ? Colors.green : Colors.black12,
                borderRadius: BorderRadius.circular(8.0),
              ),
              alignment: Alignment.center,
              child: Text(
          ...
          children: [
            ...
            Column(
              children: List.generate(
                8,
                (index) => buildGridView(index * 5 + 1, index),
              ),
            ),
            ElevatedButton(
              onPressed: numberList.every((num) => num != null)
                  ? () async {
                      await setNewBingoNums(
                          widget.no, widget.date, numberList.cast<int>());
                      Navigator.pop(context, true);
                    }
                  : null,
              child: const Text('登録'),
  ...
  try {
    await database.insert(
      'bingo',
      {
        'no': no,
        'date': date,
        'main1': numSetList[0] ?? 0,
        'main2': numSetList[1] ?? 0,
        'main3': numSetList[2] ?? 0,
        'main4': numSetList[3] ?? 0,
        'main5': numSetList[4] ?? 0,
        'main6': numSetList[5] ?? 0,
        'main7': numSetList[6] ?? 0,
        'main8': numSetList[7] ?? 0,
      },
      conflictAlgorithm: ConflictAlgorithm.ignore,
    );
  } catch (e) {
